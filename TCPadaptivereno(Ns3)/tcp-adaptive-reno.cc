...
TcpAdaptiveReno::EstimateCongestionLevel()
{
  ...
  if(m_prevConjRtt < m_minRtt) a = 0; // the initial value should take the full current Jth loss Rtt
  
  double conjRtt = a*m_prevConjRtt.GetSeconds() + (1-a)*m_jPacketLRtt.GetSeconds(); 
  m_conjRtt = Seconds(conjRtt); // for next step calculation
