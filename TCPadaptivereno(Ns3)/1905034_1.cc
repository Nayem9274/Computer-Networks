...
NS_LOG_COMPONENT_DEFINE ("1905034_1");
...
*/
  ...
  Simulator::Schedule (MilliSeconds (100), &CalculateThroughput); // calling the CalculateThroughput function periodically i.e every 100ms
}



int main(int argc, char *argv[]){
    uint nWifi = 0;  // Wifi Staion on each side of AP(access point)
    uint nFlow = 20;
    uint nNodes = 20;
    ...
    double txArea = 1.0;
    double range = 50.0; 
    std::string filename = ".scratch/1905034_1";
   ...
   // the left and right sides of the network are configured with separate wireless channels, allowing them to have independent propagation loss models 
   ...
   RwifiHelper.SetRemoteStationManager ("ns3::MinstrelHtWifiManager");
    ...
    /*This container class holds a collection of NetDevice objects. NetDevices represent the network interfaces of nodes that can send and receive data.
    This method is used to install the Wi-Fi devices on the specified nodes with the configured PHY and MAC layer settings. wifiStaNodesL and wifiStaNodesR are
    NodeContainer objects representing the left and right side Wi-Fi client nodes, respectively.*/
    ...
    // AP devices set up
    // changing the MAC type of LwifiMac and RwifiMac to "ns3::ApWifiMac". ApWifiMac stands for Access Point (AP) mode MAC, which is used for Wi-Fi access points.
    ...
    mobility.SetMobilityModel ("ns3::ConstantPositionMobilityModel"); //  since static network
    ...
    Ptr<PcapFileWrapper> file = pcapHelper.CreateFile ("scratch/1905034_1.pcap", std::ios::out, PcapHelper::DLT_PPP);
    ...
    // 
    out << nNodes << " " << nFlow << " " << nPacketsPerSecond << " " << range  << " ";
  //
